...
import { useFoodRequest } from '../../_hooks';

export default function FoodRequestPage() {
  const { form, onSubmit, isSubmitting, errors, isValid, result } = useFoodRequest();
  ...
  return (
    <div className="max-w-4xl pt-2 px-4">
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <FormField
            {...form.register('tbids_food_attrs.provision_time')}
            name="tbids_food_attrs.provision_time"
            label="Время предоставления"
            type="datetime-local"
            required
            error={errors.tbids_food_attrs?.provision_time as any}
        />
        <FormField
            {...form.register('tbids_food_attrs.adults_qty', { valueAsNumber: true })}
            name="tbids_food_attrs.adults_qty"
            label="Количество взрослых"
            type="number"
            required
            error={errors.tbids_food_attrs?.adults_qty as any}
        />
        <FormField
            {...form.register('tbids_food_attrs.kids_qty', { valueAsNumber: true })}
            name="tbids_food_attrs.kids_qty"
            label="Количество детей"
            type="number"
            error={errors.tbids_food_attrs?.kids_qty as any}
        />
        <FormField
